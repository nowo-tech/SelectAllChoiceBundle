...
1. **Product behavior** — what **SelectAllChoiceBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** enforce contracts in CI where applicable.
...
**Goal:** Symfony bundle that adds a 'Select all' checkbox for ChoiceType fields with multiple=true (expanded or select).
